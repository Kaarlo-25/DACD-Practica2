...
public class JMSReceiver implements EventSuscriptor {
	...
	public void startListening(EventStorer eventStorer) {
		try {
			TopicSubscriber consumer = createSubscriber();
			consumer.setMessageListener(message -> {
				if (message instanceof TextMessage) {
					try {
						eventStorer.storeEvents(((TextMessage) message).getText(), subject);
	...
	private TopicSubscriber createSubscriber() throws JMSException {
		...
		return session.createDurableSubscriber(destination, clientID);
	}
}
